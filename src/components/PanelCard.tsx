...
export function PanelCard({ panel }: { panel: Panel }) {
  return (
    <Card className="overflow-hidden group hover:shadow-lg transition-all duration-300 bg-card border-border/50">
      <Link href={`/panels/${panel.id}`} className="block relative aspect-[4/3] overflow-hidden">
        <Image
          src={panel.mainImage}
          alt={panel.name}
          fill
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-2">
          <div>
            <p className="text-xs font-medium text-primary uppercase tracking-wider mb-1 font-headline">
              {panel.brand}
            </p>
        ...
        <div className="grid grid-cols-2 gap-y-2 mt-4 text-sm text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            <span>{panel.width} x {panel.height} mm</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="col-span-2 flex items-center gap-2">
            <div className={`h-2 w-2 rounded-full ${panel.hasGrain ? 'bg-amber-600' : 'bg-slate-300'}`} />
            <span>{panel.hasGrain ? 'Con Vetas' : 'Liso'}</span>
          </div>
        </div>
      </CardContent>
