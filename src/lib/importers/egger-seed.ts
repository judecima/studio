...
import { ScrapedProduct } from './faplac-scraper';

/**
 * Semilla de datos para Egger.
 * Implementación MVP con códigos reales y dimensiones estándar de Egger.
 */
export async function getEggerSeedData(): Promise<ScrapedProduct[]> {
  const seed = [
  ...
  return seed.map(item => ({
    ...
    height: 2070,
    thickness: 18,
    description: `Tablero de melamina Egger con acabado ${item.texture}. Diseño de alta fidelidad para mobiliario moderno.`,
